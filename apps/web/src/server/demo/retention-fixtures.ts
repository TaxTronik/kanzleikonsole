...
async function main() {
  ...
  if (!tenant) throw new Error('Kein Tenant gefunden. Erst pnpm db:seed oder Provisioning ausfuehren.');
...
async function seedFixture(
  ...
  if (existing) {
    console.log(`[demo-retention] Bereits vorhanden: ${fixture.title} (${fixture.queueExpectation})`);
    return;
  }
  ...
  const objectLockState = await putFixtureObject(bucket, key, bytes, fixture.retainUntil);
...
async function putFixtureObject(
  ...
  try {
    ...
    return 'expired-without-active-lock';
  }
}
