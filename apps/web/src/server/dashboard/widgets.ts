...
export const WIDGETS: WidgetDef[] = [
  ...
  { type: 'kpi_open_requests', label: 'Offene Anforderungen', description: 'Anforderungen mit Status OPEN/IN_PROGRESS', size: 'kpi' },
  { type: 'kpi_documents', label: 'Dokumente', description: 'Gesamtzahl Dokumente', size: 'kpi' },
  { type: 'kpi_unread_notes', label: 'Offene Telefonzettel', description: 'Telefonnotizen, die noch nicht erledigt sind', size: 'kpi' },
  { type: 'kpi_pending_change_requests', label: 'Offene Stammdaten-Anträge', description: 'Mandanten-Änderungsanträge', size: 'kpi' },
  { type: 'kpi_open_workflows', label: 'Laufende Workflows', description: 'Aktive Workflow-Instanzen', size: 'kpi' },
  { type: 'recent_activity', label: 'Letzte Aktivitäten', description: 'Audit-Strom (Top 10)', size: 'wide' },
  { type: 'upcoming_requests', label: 'Fällige Anforderungen', description: 'Anforderungen mit Fälligkeit', size: 'half' },
  { type: 'gwg_expiring', label: 'GwG läuft bald aus', description: 'Mandanten mit GwG-Ablauf < 90 Tage', size: 'half' },
  { type: 'unreviewed_notices', label: 'Ungeprüfte Bescheide', description: 'Bescheide mit Status NEU', size: 'half' },
  { type: 'my_tax_deadlines', label: 'Nächste Steuertermine', description: 'Anstehende Termine der Kanzlei', size: 'half' },
  { type: 'calendar', label: 'Kalender', description: 'Anstehende Termine + Steuertermine der nächsten Tage', size: 'half' },
  { type: 'tax_news', label: 'RSS-Reader', description: 'Eigene RSS-Feeds (BMF, BFH und beliebige weitere)', size: 'half' },
  { type: 'phone_notes', label: 'Telefonzettel', description: 'Ungelesene + aktuelle Telefonnotizen der Kanzlei', size: 'half' },
  { type: 'bookmarks', label: 'Gemerkt', description: 'Persönlich gemerkte Einträge (z. B. BMF/BFH-Schreiben)', size: 'half' },
  { type: 'personal_notes', label: 'Persönliche Notizen', description: 'Eigene Kurz-Notizen — privater Block', size: 'half' },
  { type: 'my_workflow_items', label: 'Mein Tag', description: 'Offene Workflow-Schritte, die mir zugewiesen sind', size: 'half' },
  { type: 'my_workflows', label: 'Meine Workflows', description: 'Workflow-Instanzen, die mir Schritte zuweisen oder die ich gestartet habe', size: 'half' },
  { type: 'my_reminders', label: 'Wiedervorlagen', description: 'Mir zugewiesene offene Wiedervorlagen, fällig zuerst', size: 'half' },
];

export const WIDGET_BY_TYPE: Record<WidgetType, WidgetDef> =
  Object.fromEntries(WIDGETS.map((w) => [w.type, w])) as Record<WidgetType, WidgetDef>;
...
export const DEFAULT_SIZE: Record<WidgetType, { w: number; h: number; minW: number; minH: number }> = {
...
export function parseLayout(raw: unknown): DashboardLayout {
  ...
  if (obj.version !== 2) {
    ...
    for (const w of obj.widgets) {
      ...
      if (x + def.w > 12) { x = 0; y += rowH; rowH = 0; }
