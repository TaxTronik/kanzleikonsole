...
describe('kuratiereKatalogNorm', () => {
  ...
  it('wirft NotACatalogMarkingError ohne begriffId — kein Engine-Call, kein Audit', async () => {
    ...
    const client = { katalogNormKuratieren: vi.fn(async () => ({ ok: true })) };
    await expect(
      kuratiereKatalogNorm(ctx, input, client as unknown as Client),
    ).rejects.toBeInstanceOf(NotACatalogMarkingError);
  ...
  it('wirft CatalogCurationFailedError bei ok:false und auditiert NICHT', async () => {
    const client = {
      katalogNormKuratieren: vi.fn(async () => ({ ok: false, fehler: 'unbekannte katalog_id' })),
    };
    await expect(kuratiereKatalogNorm(ctx, input, client as unknown as Client)).rejects.toThrow(
      'unbekannte katalog_id',
    );
    expect(h.record).not.toHaveBeenCalled();
  });
});
