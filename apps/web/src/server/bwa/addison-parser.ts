 ...
 * (1020 Umsatzerlöse, 1051 Gesamtleistung, 1100 Personalkosten, 1300 Betriebsergebnis,
...
export interface BwaKpis {
  ...
  costs: number | null;
  result: number | null;
  /** Ergebnis vor Ertragsteuern (DATEV 1345). Betriebsergebnis oder
   * vorläufiges DATEV-Ergebnis ersetzen eine fehlende Position nicht. */
...
const DATEV_REVENUE = 1020; // Umsatzerlöse
const DATEV_TOTAL_OUTPUT = 1051; // Gesamtleistung einschließlich Bestandsänderungen/Eigenleistungen
const DATEV_MATERIAL = 1060;
const DATEV_OTHER_OPERATING_INCOME = 1090;
const DATEV_TOTAL_COSTS = 1280; // Kostenarten ohne Material-/Wareneinkauf
const DATEV_RESULT = 1380; // Vorläufiges Ergebnis
const DATEV_RESULT_BEFORE_TAX = 1345; // Ergebnis vor Steuern
const DATEV_PERSONNEL = 1100;
const DATEV_OPERATING_RESULT = 1300; // Betriebsergebnis vor neutralen Aufwendungen/Erträgen

function datevOperatingCosts(positions: ReadonlyMap<number, number>): number | null {
  const material = positions.get(DATEV_MATERIAL);
  const costs = positions.get(DATEV_TOTAL_COSTS);
  if (material !== undefined && costs !== undefined) return material + costs;

  const output = positions.get(DATEV_TOTAL_OUTPUT);
  const otherIncome = positions.get(DATEV_OTHER_OPERATING_INCOME);
  const operatingResult = positions.get(DATEV_OPERATING_RESULT);
  if (output === undefined || otherIncome === undefined || operatingResult === undefined) {
    return null;
  }
  return output + otherIncome - operatingResult;
}
...
export function computeBwaKpis(
  ...
  // Vor-Steuer-Ergebnis: ausschließlich DATEV 1345. 1300 enthält die neutralen
  // Aufwendungen/Erträge noch nicht. Die Addison-Kompaktform ("Kurzfristige Erfolgsrechnung")
  ...
  const resultBeforeTax = map.get(DATEV_RESULT_BEFORE_TAX) ?? map.get(ADDISON_RESULT) ?? null;

  // DATEV: betriebliche Aufwendungen einschließlich Material. Ein fehlender
  // Bestandteil ist unbekannt; nur ein ausdrücklich vorhandener Wert zählt als 0.
  const costs = map.get(ADDISON_COSTS) ?? datevOperatingCosts(map);
