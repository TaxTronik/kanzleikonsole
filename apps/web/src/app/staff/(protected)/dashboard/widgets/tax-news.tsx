    ...
    <div className="card h-full flex flex-col">
      <div className="px-5 py-3 border-b border-default flex items-center justify-between gap-2 shrink-0 relative">
        ...
        <div className="flex items-center gap-1 relative">
          ...
          />
          <TaxNewsToggle enabled={Boolean(staff?.taxNewsNotify)} canTriggerFetch={Boolean(isAdmin)} />
        </div>
      </div>
      {activeUrls.length === 0 ? (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <p className="text-sm font-medium text-primary line-clamp-2 flex-1">{n.title}</p>
                      <ExternalLink className="h-3 w-3 text-disabled shrink-0 mt-1" />
                    </div>
                    <p className="text-xs text-muted mt-0.5">
                      <span className={'inline-block rounded px-1.5 py-0.5 mr-1.5 text-[10px] font-medium ' + feedBadgeClass(color)}>
                        {label}
                      </span>
                      {n.publishedAt
