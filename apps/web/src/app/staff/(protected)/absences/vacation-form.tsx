  ...
  return (
    <form ref={formRef} action={formAction} className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="label" htmlFor="vac-startDate">
            Von
          </label>
          <input id="vac-startDate" name="startDate" type="date" className="input" required />
        </div>
        <div>
          <label className="label" htmlFor="vac-endDate">
            Bis
          </label>
      ...
      <div>
        <label className="label" htmlFor="vac-reason">
          Grund (optional)
        </label>
        <textarea id="vac-reason" name="reason" rows={2} className="input" maxLength={1000} />
      </div>
      {state?.error && <div className="alert-error-sm">{state.error}</div>}
      {state?.ok && <div className="alert-success-sm">Antrag gestellt.</div>}
