...
import { confirmDialog, Modal, noticeDialog } from '@/components/ui/modal';
...
export function PauseWorkflowButton({
  ...
  const modal = open ? (
    ...
    >
      ...
      <div>
        <label className="label" htmlFor={`pause-workflow-${instanceId}-until`}>
          Wiederaufnahme am (optional)
        </label>
        <input
          id={`pause-workflow-${instanceId}-until`}
          type="date"
          value={until}
          onChange={(e) => setUntil(e.target.value)}
          min={new Date().toISOString().slice(0, 10)}
          className="input text-sm"
          aria-describedby={`pause-workflow-${instanceId}-until-hint`}
        />
        <p id={`pause-workflow-${instanceId}-until-hint`} className="text-[10px] text-muted mt-1">
          Wenn gesetzt, wird der Workflow ab diesem Tag automatisch wieder aktiv. Leer = unbefristet
          pausiert, muss manuell fortgesetzt werden.
        </p>
      </div>
      <div>
        <label className="label" htmlFor={`pause-workflow-${instanceId}-reason`}>
          Grund (optional)
        </label>
        <textarea
          id={`pause-workflow-${instanceId}-reason`}
          value={reason}
...
export function ResumeWorkflowButton({ instanceId }: { instanceId: string }) {
  ...
  function go() {
    start(async () => {
      ...
      else await noticeDialog(r.error ?? 'Fehler.', { title: 'Workflow fortsetzen' });
    });
  }
  return (
    <button
...
export function RestoreWorkflowButton({ instanceId }: { instanceId: string }) {
  ...
  async function go(e: MouseEvent) {
    ...
    e.stopPropagation();
    if (
      !(await confirmDialog('Workflow wiederherstellen? Status geht zurück auf "Aktiv".', {
        title: 'Workflow wiederherstellen',
    ...
    start(async () => {
      ...
      if (r.ok) router.refresh();
      else await noticeDialog(r.error ?? 'Fehler.', { title: 'Workflow wiederherstellen' });
    });
  }
  return (
    <button
