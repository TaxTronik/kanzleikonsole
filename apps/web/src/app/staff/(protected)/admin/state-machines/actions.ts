...
export async function createStateMachineAction(
  ...
  try {
    id = await withTenantContext(ctx, async (tx) => {
      ...
      if (dup) throw new ActionError('Slug bereits vergeben.');
  ...
  revalidatePath(LIST);
  redirect(`${LIST}/${id}`); // wirft (never) — bewusst NACH dem try/catch
}
...
export async function updateMachineMetaAction(
  ...
  return withStaff(
    async (tx, { tenantId, staffId }) => {
      ...
      if (!before) throw new ActionError('Status-Maschine nicht gefunden.');
    ...
    { requireAdmin: true, revalidate: [`${LIST}/${parsed.data.id}`, LIST] },
  );
}
...
export async function deleteMachineAction(input: { id: string }): Promise<ActionResult> {
  ...
  return withStaff(
    async (tx, { tenantId, staffId }) => {
      ...
      if (!m) throw new ActionError('Status-Maschine nicht gefunden.');
    ...
    { requireAdmin: true, revalidate: LIST },
  );
}
...
export async function saveMachineDefinitionAction(
  ...
  return withStaff(
    async (tx, { tenantId, staffId }) => {
      ...
      if (!machine) throw new ActionError('Status-Maschine nicht gefunden.');
      ...
      if (toRemove.length > 0) {
        await tx.stateMachineState.deleteMany({ where: { id: { in: toRemove.map((s) => s.id) } } });
      }
    ...
    { requireAdmin: true, revalidate: `${LIST}/${machineId}` },
  );
}
