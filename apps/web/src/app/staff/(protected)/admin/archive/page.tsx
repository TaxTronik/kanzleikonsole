    ...
    <div className="p-8 max-w-5xl">
      <div className="flex items-start gap-4 mb-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-gray-500 text-sm">
            ...
            und mit Object-Lock COMPLIANCE für 10 Jahre in MinIO gespeichert. Wird vom
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="card p-4">
          <p className="text-xs text-gray-500 uppercase">Archiv-Segmente</p>
          <p className="text-3xl font-bold text-gray-900 mt-1">{data.archives.length}</p>
        </div>
        <div className="card p-4">
          <p className="text-xs text-gray-500 uppercase">Archivierte Einträge</p>
          <p className="text-3xl font-bold text-gray-900 mt-1">{data.totalArchived.toLocaleString('de-DE')}</p>
        </div>
        <div className="card p-4">
          <p className="text-xs text-gray-500 uppercase">Im DB-Log offen</p>
          <p className="text-3xl font-bold text-gray-900 mt-1">{data.pendingCount.toLocaleString('de-DE')}</p>
          <p className="text-xs text-gray-400 mt-1">noch nicht archiviert</p>
        </div>
      </div>

      <div className="card overflow-hidden">
        <div className="px-6 py-3 border-b border-gray-200 text-xs text-gray-500 flex items-center justify-between">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase">Einträge</th>
                <th className="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase">Zeitraum</th>
                <th className="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase">Größe</th>
                <th className="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase">Modus</th>
                <th className="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase">Datei-SHA-256</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {data.archives.map((a) => (
                <tr key={String(a.id)} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-3 text-gray-700">{a.entryCount.toLocaleString('de-DE')}</td>
                  <td className="px-6 py-3 text-xs text-gray-600">
                    {dateFmt.format(a.fromOccurredAt)}
                    <br />
                    <span className="text-gray-400">bis</span> {dateFmt.format(a.toOccurredAt)}
                  </td>
      ...
      <p className="text-xs text-gray-400 mt-4 flex items-center gap-2">
        ...
        Jedes Segment ist als NDJSON-Datei mit RFC-3161-Stempel in MinIO archiviert
