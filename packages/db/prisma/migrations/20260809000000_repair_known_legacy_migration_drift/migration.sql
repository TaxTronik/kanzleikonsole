-- Forward-only repair for databases that applied known pre-release versions of
-- migrations 03400-04400 before those files reached their canonical form.
--
-- The production ledger gate accepts only the exact legacy checksums listed
-- below, applies this migration, and verifies the complete ledger again. The
-- DDL is deliberately idempotent so fresh installations and partially affected
-- beta installations converge on the same protection schema.
