...
| n8n-Callbacks v1  | Key-ID löst serverseitig genau eine `N8nConnection`/einen Tenant auf; separater Bearer-Token und minimale Scopes, übergebene `tenantId`-Werte werden abgelehnt                                                                                             | ✓                                   | ✓ — tenantgebundener Standardpfad                                                                               |
| n8n-Legacy-HMAC   | Default-off (`N8N_LEGACY_CALLBACKS_ENABLED=false`, `N8N_WEBHOOK_BASE_URL` leer); ein globales `N8N_HMAC_SECRET` ist nur beim befristeten Callback- oder Outbound-Opt-in aktiv, nicht tenantgebunden und dann produktiv mit mindestens 32 Zeichen erzwungen | ⚠ Nur dokumentierter Migrationspfad | ✓ Im Default; ausschließlich workflow-spezifische Ziele und `/api/integrations/n8n/v1/*` nutzen                 |
