...
- Art. 6 Abs. 1 lit. f DSGVO: berechtigtes Interesse der Kanzlei an
  ...
  behörden. Diese Interessenabwägung setzt eine ereignisbezogene
  Datenminimierung voraus. Der Audit-Log enthält neben pseudonymen IDs je nach
  Aktion auch technische Metadaten und optionale `before`/`after`-Werte; eine
  pauschale Aussage „nur actor_id + IP/User-Agent“ oder „keine Inhalte“ wäre
  daher nicht zutreffend. Namen, Freitext und Geschäftsvorfallsinhalte sollen
  nicht dupliziert werden; wo ein Inhaltsbezug nötig ist, sind Statuscodes,
  boolesche Nachweise oder ein gebundener Hash vorzuziehen.
...
deshalb keine Empfänger-/Behördennamen, Ortsangaben, Ereignisdaten oder
Begründungsfreitexte in `before`/`after`, sondern pseudonyme Referenzen,
Status-/Nachweismerkmale und einen an die Ressourcen-ID gebundenen
SHA-256-Fingerprint des Fachdatensatzes beziehungsweise der Fachänderung. Der Klartext bleibt im Fachobjekt mit
dessen eigener Aufbewahrungs- und Löschlogik. Diese Härtung ersetzt nicht die
weiterhin erforderliche Prüfung aller übrigen Audit-Aufrufer.
...
| Pseudonyme Steuertermin-Versandhistorie nach Terminlöschung                              | spätestens 1 Jahr nach Archivierung; früher bei Löschung der verknüpften Anforderung                                       | Worker `dsgvo-retention` tenantgebunden; Fehlerklartext wird nicht archiviert, der App-Rolle fehlen Tabellenrechte                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
> **Offene Retention für Fristen-Tagesabschlüsse:**
> `deadline_daily_review` ist app-seitig append-only. Der Eintragssnapshot ist
> per DB-Positivliste auf Quelle, technische Kontrollart, Fälligkeit und
> pseudonyme UUIDs begrenzt und
> enthält keine Namen oder Fachtitel; die freie Eskalationsnotiz kann dennoch
> personenbezogen sein. Eine Löschautomatik und allgemeine Höchstfrist sind
> noch nicht implementiert und müssen kanzleispezifisch freigegeben werden.
>
...
- Eine nachträgliche **Pseudonymisierung einzelner Einträge ist nicht ohne
  Bruch der Hash-Chain möglich**. Personenbezogene Klarwerte müssen deshalb
  bereits beim Schreiben vermieden werden; Akteurs- und Ressourcenbezüge
  bleiben soweit möglich pseudonyme UUIDs.
...
| Bescheid angelegt / Status nachgewiesen          | `tax_notice`; Audit nur mit pseudonymen Referenzen, Status-/Nachweismerkmalen und gebundenem Datensatz-/Änderungsfingerprint, ohne Namen oder Begründungsfreitext                                                                                                                                                                                                                                                      | `tax_notice.create` / `.status` |
| Fristen-Tagesabschluss                           | `deadline_daily_review` (append-only; Snapshot ohne Namen/Fachtitel, mit pseudonymen UUIDs und optionaler Eskalationsnotiz)                                                                                                                                                                                                                                                                                            | `fristen.daily_review.complete` |
