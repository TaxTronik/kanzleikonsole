...
| Korrekturbeleg für In-App-Rechnung      | `cancelInvoiceAction` + `xrechnung.ts`                               | `storno.test.ts` (negative Positionen) und `xrechnung.test.ts` (TypeCode 381/Vorgängerreferenz); Race-Helfer sind direkt getestet, der vollständige Action-Orchestrierungsfluss bleibt als offene Nachweislücke im Testkonzept ausgewiesen                                         |
