...
`DocumentType` (Admin-gepflegt; Schutzstufe bei Anlage fixiert, 7 Kern-Typen
read-only).
...
| Kontrolle | Umsetzung |
|---|---|
| Annahme-Pipeline | zentral `packages/storage/src/service.ts`: ClamAV-Scan **synchron vor** jedem Upload (INFECTED→422, Scanner-Fehler→502, fail-closed — nichts wird gespeichert), SHA-256, Magic-Byte-MIME-Vorrang, 100-MB-Cap (Teilbereiche 10 MB) |
| Unveränderlichkeit | Object-Lock je Stufe: GOBD→COMPLIANCE 10 J. (§ 147 AO), GWG→GOVERNANCE 5 J. (§ 8 Abs. 4 GwG, Vernichtungspflicht); DB-Trigger `document_version_immutable_protect` (auch Owner); Herabstufung der Schutzstufe blockiert, Höherstufung re-stored in den gelockten Bucket |
